...
// File upload for CSS/JS
const uploadCssBtn  = document.getElementById('uploadCssBtn');
const uploadJsBtn   = document.getElementById('uploadJsBtn');
...
function toggleComment(cm) {
  ...
  if (hasSelection) {
    ...
    if (isCss) {
      ...
      if (allCommented) {
        ...
        const uncommented = lines.map(line => {
          ...
          return ' '.repeat(spaces) + trimmed.replace(/^\/\/\s?/, '');
        }).join('\n');
    ...
    if (isCss) {
      // CSS single line toggle
      if (trimmed.startsWith('/*') && trimmed.endsWith('*/')) {
        // Uncomment
        const uncommented = ' '.repeat(spaces) + trimmed.slice(2, -2).trim();
        cm.replaceRange(uncommented, { line: cursor.line, ch: 0 }, { line: cursor.line, ch: line.length });
      } else {
        // Comment
        const commented = ' '.repeat(spaces) + '/* ' + trimmed + ' */';
        cm.replaceRange(commented, { line: cursor.line, ch: 0 }, { line: cursor.line, ch: line.length });
      }
    } else {
      // JavaScript single line toggle
      if (trimmed.startsWith('//')) {
        // Uncomment
        const uncommented = ' '.repeat(spaces) + trimmed.replace(/^\/\/\s?/, '');
        cm.replaceRange(uncommented, { line: cursor.line, ch: 0 }, { line: cursor.line, ch: line.length });
      } else {
        // Comment
        const commented = ' '.repeat(spaces) + '// ' + trimmed;
...
function createCssEditor() {
  ...
  // Initialize color picker after editor is ready
  initColorPicker();
}
...
function setupEventListeners() {
  ...
  if (exportFilesBtn) {
    exportFilesBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      const dropdown = exportFilesBtn.closest('.export-dropdown');
  ...
  document.querySelectorAll('.export-dropdown-item').forEach(item => {
    item.addEventListener('click', () => {
      const shouldMinify = item.dataset.minify === 'true';
      handleExportAsFiles(shouldMinify);
  ...
  document.addEventListener('click', () => {
    document.querySelector('.export-dropdown.active')?.classList.remove('active');
  });
  ...
  syncToggle.addEventListener('change', handleSyncToggle);
   layoutSelect.addEventListener('change', async () => {
    ...
    showStatus('🔄 Switching to Layout ' + settings.layout.toUpperCase() + '...', 'info');
    setTimeout(() => {
      const layoutPaths = {
        'a': '../panel-a/panel.html',
        'b': '../panel-b/panel.html',
        'c': '../panel-c/panel.html'
      };
      window.location.replace(layoutPaths[settings.layout]);
    }, 800);
  });
  ...
  // CSS/JS file upload
  if (uploadCssBtn) uploadCssBtn.addEventListener('click', () => cssFileInput.click());
  if (uploadJsBtn) uploadJsBtn.addEventListener('click', () => jsFileInput.click());
  if (cssFileInput) cssFileInput.addEventListener('change', handleCssFileUpload);
  if (jsFileInput) jsFileInput.addEventListener('change', handleJsFileUpload);

  // Clear editors
  if (clearCssBtn) clearCssBtn.addEventListener('click', handleClearCssEditor);
  if (clearJsBtn) clearJsBtn.addEventListener('click', handleClearJsEditor);
...
async function handleAutoInjectToggle() {
  ...
  // Reload the inspected tab so the change takes effect immediately
  const tabId = getInspectedTabId();
...
async function handleCssFileUpload(e) {
  const file = e.target.files[0];
  if (!file) return;
  try {
    const content = await file.text();
    if (cssEditorCM) {
      cssEditorCM.setValue(content);
      showStatus(`📁 ${file.name} loaded into CSS editor`, 'success');
    }
    cssFileInput.value = '';
  } catch (error) {
    showStatus('❌ Failed to read CSS file: ' + error.message, 'error');
  }
}

async function handleJsFileUpload(e) {
  const file = e.target.files[0];
  if (!file) return;
  try {
    const content = await file.text();
    if (jsEditorCM) {
      jsEditorCM.setValue(content);
      showStatus(`📁 ${file.name} loaded into JS editor`, 'success');
    }
    jsFileInput.value = '';
  } catch (error) {
    showStatus('❌ Failed to read JS file: ' + error.message, 'error');
  }
}

function handleClearCssEditor() {
  if (!confirm(t('confirmClearCssEditor'))) return;
  if (cssEditorCM) {
    cssEditorCM.setValue('');
    showStatus('🗑️ CSS editor cleared', 'success');
  }
}

function handleClearJsEditor() {
  if (!confirm(t('confirmClearJsEditor'))) return;
  if (jsEditorCM) {
    jsEditorCM.setValue('');
    showStatus('🗑️ JS editor cleared', 'success');
  }
}
...
// ─────────────────────────────────────────
//  Color Picker Feature (Stable Version)
// ─────────────────────────────────────────


// ═══════════════════════════════════════════════════════════
// COLOR PICKER - Now in lib/panel-common.js
// ═══════════════════════════════════════════════════════════
// The following variables and functions have been moved to panel-common.js:
// - colorMarkers, isUpdatingMarkers, currentColorMark, markerUpdateTimeout
// - COLOR_REGEX
// - addColorMarkers(), colorToHex(), openColorPicker(), closeColorPicker(), initColorPicker()
// ═══════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════
//  EXPORT AS FILES - Export snippets as CSS/JS files
// ═══════════════════════════════════════════════════════════
...
/**
 * Export current profile snippets as CSS and JS files
 */
async function handleExportAsFiles(shouldMinify = false) {
  if (!currentProfile) {
    showStatus('No profile loaded', 'error');
    return;
  }

  // Get all enabled snippets (include default css/js if snippets array is empty)
  const enabled = currentProfile.snippets?.filter(s => s.enabled !== false);
  const snippets = enabled && enabled.length > 0
    ? enabled
    : [{ css: currentProfile.css || '', js: currentProfile.js || '' }];

  if (snippets.length === 0) {
    showStatus('No snippets to export', 'error');
    return;
  }

  // Combine all CSS
  const allCSS = snippets
    .map((s, idx) => {
      const comment = s.name ? '/* Snippet: ' + s.name + ' */\n' : '/* Snippet ' + (idx + 1) + ' */\n';
  ...
  const safeDomain = currentDomain.replace(/[^a-z0-9]/gi, '-').toLowerCase();

  let finalCSS = allCSS.trim();
  let finalJS = allJS.trim();

  // Minify if enabled
  if (shouldMinify) {
    try {
      // Minify CSS using csso
      if (finalCSS && typeof csso !== 'undefined') {
        finalCSS = csso.minify(finalCSS).css;
      }

      // Minify JS using Terser
      if (finalJS && typeof Terser !== 'undefined') {
        const result = await Terser.minify(finalJS, {
      ...
      showStatus('⚠️ Minification failed, exporting original', 'error');
    }
  }
  ...
  if (finalCSS) {
    zip.file('overrides.css', finalCSS);
  }

  if (finalJS) {
    zip.file('overrides.js', finalJS);
  }

  // Generate and download ZIP via background script (devtools can't trigger downloads directly)
  const base64 = await zip.generateAsync({ type: 'base64' });
  const dataUrl = 'data:application/zip;base64,' + base64;
  await chrome.runtime.sendMessage({
    type: 'DOWNLOAD_FILE',
    data: { dataUrl, filename: safeDomain + '-overrides.zip' }
  });

  const statusMsg = shouldMinify ? '✅ Exported (Minified)' : '✅ Exported as ZIP';
